...
public class OutlineListAdt implements ListAdapter {
    ...
    private List<outline_ui_item> m_itemsList = new ArrayList<>();
    public void addItem(outline_ui_item item) {
        m_itemsList.add(item);
        m_items = m_itemsList.toArray(new outline_ui_item[0]);
        if (m_obs != null)
            m_obs.onChanged();
    }
    ...
    private static OutlineRec m_cur;
    ...
    public class outline_ui_item implements View.OnClickListener {
        ...
        outline_ui_item(Context ctx, Document.Outline outline) {
            m_type = 1;
            m_outline = outline;
        ...
        outline_ui_item(Context ctx, String title, int pageNo) {
            m_type = 0;
            m_outline = null;
            m_title = title;
            m_pageNo = pageNo;
    ...
    private void refresh() {
        ...
        if (m_cur == null) {
            for (Document.Outline root = m_doc.GetOutlines(); root != null; root = root.GetNext()) {
                m_itemsList.add(new outline_ui_item(m_ctx, root));
            }
        } else {
            m_itemsList.add(new outline_ui_item(m_ctx, 0, m_ctx.getString(R.string.text_back_label)));
            for (Document.Outline item = m_cur.m_parent_outline.GetChild(); item != null; item = item.GetNext()) {
                m_itemsList.add(new outline_ui_item(m_ctx, item));
            }
        }
        refreshItems();
    }
